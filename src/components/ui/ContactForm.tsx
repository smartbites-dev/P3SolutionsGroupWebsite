...
const labelClass = 'block text-xs font-medium uppercase tracking-wide text-zinc-400';
  ...
  if (status === 'success') {
    return (
      <div className="flex flex-col items-start rounded-xl border border-p3-red/25 bg-p3-red/[0.04] p-8">
        ...
        <h4 className="mt-4 text-lg text-white">Message received.</h4>
        <p className="mt-2 leading-relaxed text-zinc-400">
          Thanks for reaching out — we'll get back to you shortly. If it's time-sensitive, call{' '}
          <a href={site.phoneHref} className="font-medium text-p3-red hover:text-white">
            {site.phone}
          </a>
          .
        </p>
        <button
          ...
          className="mt-6 text-sm font-semibold text-p3-red transition-colors hover:text-white"
        >
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-4 pt-1">
        ...
        <p className="text-xs text-zinc-400">We'll only use this to reply. No lists, no sequences.</p>
      </div>

      <div aria-live="polite">
        {status === 'error' && (
          <p className="flex items-start gap-2.5 rounded-lg border border-p3-red/30 bg-p3-red/[0.04] p-4 text-sm text-p3-red">
