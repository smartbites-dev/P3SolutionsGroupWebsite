    ...
    >
      <div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-[1.1fr_1fr]">
        <div className="rounded-2xl border border-zinc-200 bg-white p-8 lg:p-10">
          ...
          <div className="mt-8 flex flex-col gap-3 border-t border-zinc-100 pt-8 sm:flex-row">
            <Button href={mailto('AI Operating System — initial conversation')}>Talk With P3</Button>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-p3-mist p-8 lg:p-10">
          <h3 className="text-xl">A good fit looks like</h3>
          <ul className="mt-6 space-y-4">
            {goodFit.map((item) => (
              <li key={item} className="flex items-start gap-3 leading-relaxed text-zinc-600">
                <span
                  aria-hidden="true"
                  className="mt-[9px] h-1.5 w-1.5 shrink-0 rounded-full bg-p3-red"
                />
                {item}
              </li>
            ))}
