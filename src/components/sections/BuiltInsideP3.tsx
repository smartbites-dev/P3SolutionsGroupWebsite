...
export function BuiltInsideP3() {
  return (
    <section id="built-inside" className="scroll-mt-24 border-y border-zinc-200 bg-white py-20 lg:py-28">
      <Container>
        <div className="grid items-start gap-14 lg:grid-cols-2 lg:gap-20">
          <div>
            <Eyebrow>Built Inside P3</Eyebrow>
            ...
            {path.map((item, i) => (
              <li key={item.step} className="relative flex gap-6 pb-8 last:pb-0">
                {i < path.length - 1 && (
                  <span
                    aria-hidden="true"
                    className="absolute left-[19px] top-11 h-[calc(100%-1.75rem)] w-px bg-zinc-200"
                  />
                )}
                <span className="relative z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-zinc-200 bg-white font-mono text-xs font-medium text-p3-red">
                  0{i + 1}
                </span>
                <div className="pt-1.5">
                  <h3 className="text-base">{item.step}</h3>
                  <p className="mt-1.5 text-sm leading-relaxed text-zinc-600">{item.detail}</p>
                </div>
              </li>
            ))}
          </ol>
